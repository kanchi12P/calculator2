...
(function () {
  ...
  core.Symbol.prototype.hasNegativeTerms = function () {
    if (this.isComposite()) {
      for (var x in this.symbols) {
        ...
        if (
          (sym.group === PL && sym.hasNegativeTerms()) ||
          this.symbols[x].power.lessThan(0)
        )
          return true;
      }
    }
  ...
  function Equation(lhs, rhs) {
    if (
      ...
      lhs.equals(core.Settings.IMAGINARY)
    )
  ...
  var toLHS = function (eqn) {
    ...
    if (eqn instanceof Equation) return eqn.toLHS();
    var es = eqn.split("=");
  ...
  var cubic = function (d_o, c_o, b_o, a_o) {
    ...
    var d0s = "({1})^2-3*({0})*({2})",
      ...
      Q = _.parse(
        format(
          "((2*({1})^3-9*({0})*({1})*({2})+27*({0})^2*({3}))^2-4*(({1})^2-3*({0})*({2}))^3)^(1/2)",
          a,
          b,
          c,
          d
      ...
      C = _.parse(
        format(
          "((1/2)*(({4})+2*({1})^3-9*({0})*({1})*({2})+27*({0})^2*({3})))^(1/3)",
          a,
          b,
          c,
          d,
    ...
    var xs = [
      ...
      "-(b/(3*a))+(C*(1+i*sqrt(3)))/(6*a)+((1-i*sqrt(3))*(b^2-3*a*c))/(6*a*C)".replace(
        /i/g,
        core.Settings.IMAGINARY
      ),
      "-(b/(3*a))+(C*(1-i*sqrt(3)))/(6*a)+((1+i*sqrt(3))*(b^2-3*a*c))/(6*a*C)".replace(
        /i/g,
        core.Settings.IMAGINARY
  ...
  var quartic = function (e, d, c, b, a) {
    ...
    p = _.parse(
      format("(8*({0})*({2})-3*({1})^2)/(8*({0})^2)", a, b, c)
    ).toString(); //a, b, c
    q = _.parse(
      format(
        "(({1})^3-4*({0})*({1})*({2})+8*({0})^2*({3}))/(8*({0})^3)",
        a,
        b,
        c,
        d
    ...
    D0 = _.parse(
      format("12*({0})*({4})-3*({1})*({3})+({2})^2", a, b, c, d, e)
    ).toString(); //a, b, c, d, e
    D1 = _.parse(
      format(
    ...
    Q = _.parse(
      format("((({1})+(({1})^2-4*({0})^3)^(1/2))/2)^(1/3)", D0, D1)
    ).toString(); //D0, D1
    S = _.parse(
      format(
        "(1/2)*(-(2/3)*({1})+(1/(3*({0}))*(({2})+(({3})/({2})))))^(1/2)",
        a,
        p,
        Q,
        D0
  ...
  var solve = function (eqns, solve_for, solutions) {
    ...
    var get_points = function (symbol) {
      ...
      for (var i = start; i < core.Settings.solve_radius; i++) {
        ...
        if (isNaN(val) || !isFinite(val) || points.length > rside) break;
      ...
      for (var i = start - 1; i > -core.Settings.solve_radius; i--) {
        var val = f(i),
          sign = val / Math.abs(val);
        if (isNaN(val) || !isFinite(val) || points.length > lside) break;
        //compare the signs. The have to be different if they cross a zero
        if (sign !== last_sign) points.push((i - 1) / 2); //take note of the possible zero location
        last_sign = sign;
      }
            ...
            if (p.den.gt(1)) {
              ...
              if (min_p < 0) {
                ...
                original.each(function (x) {
                  corrected = _.add(
                    corrected,
                    _.multiply(x.clone(), factor.clone())
                  );
                }, true);
                return corrected;
              }
    ...
    var rewrite = function (rhs, lhs) {
      ...
      if (rhs.group === CP && rhs.contains(solve_for) && rhs.isLinear()) {
        ...
        //a*x^2+x
        if (c !== l) return rewrite(rhs, lhs);
        else {
          return [rhs, lhs];
        }
      } else if (
        rhs.group === CB &&
        rhs.contains(solve_for) &&
        rhs.isLinear()
      ) {
        ...
        if (lhs.equals(0)) return new Symbol(0);
          ...
          var t = new Symbol(1);
          rhs.each(function (x) {
            if (x.contains(solve_for)) t = _.multiply(t, x.clone());
      ...
      if (eq.isPoly(true)) {
        ...
        if (vars[0] === solve_for) {
          ...
          if (deg === 1) add_to_result(_.divide(coeffs[0], coeffs[1].negate()));
            ...
            coeffs.push("-");
            add_to_result(_.expand(quad.apply(undefined, coeffs)));
          } else if (deg === 3) add_to_result(cubic.apply(undefined, coeffs));
      ...
      if (!eq.hasFunc(solve_for) && eq.isComposite()) {
        ...
        try {
          ...
          if (lhs.group === FN) {
            ...
            } else solutions.push(_.subtract(lhs, rhs));
          }
        } catch (error) {
          ...
          try {
            ...
            if (eq.group === CB) solutions.push(0);
            else if (eq.group === CP) {
              var separated = separate(eq);
              var lhs = separated[0],
                rhs = separated[1];
  ...
  core.Expression.prototype.solveFor = function (x) {
    return solve(
      core.Utils.isSymbol(this.symbol) ? this.symbol : this.symbol.toLHS(),
      x
    ).map(function (x) {
  ...
  core.Expression.prototype.variables = function () {
    if (this.symbol instanceof Equation)
      return core.Utils.arrayUnique(
        variables(this.symbol.LHS).concat(variables(this.symbol.RHS))
      );
