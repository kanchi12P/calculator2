...
(function () {
  ...
  Polynomial.prototype = {
    ...
    parse: function (symbol, c) {
      ...
      if (!symbol.isPoly())
        throw new Error(
          "Polynomial Expected! Received " + core.Utils.text(symbol)
        );
      c = c || [];
      ...
      if (symbol.group === core.groups.N) {
        ...
        for (var x in symbol.symbols) {
          ...
          if (core.Utils.isSymbol(p))
            throw new Error("power cannot be a Symbol");
    ...
    gcf: function (toPolynomial) {
      ...
      var p = [
        core.Math2.QGCD.apply(undefined, ca),
        fnz(this.coeffs)
      ].toDecimal();
    ...
    quad: function (incl_img) {
      ...
      if (this.coeffs.length > 3)
        throw new Error(
          "Cannot calculate quadratic order of " + (this.coeffs.length - 1)
        );
  ...
  Symbol.prototype.coeffs = function (c, with_order) {
    ...
    if (s.isComposite()) {
      ...
      if (with_order)
        c[s.isConstant() ? 0 : s.power.toDecimal()] = s.multiplier;
  ...
  Factors.prototype.add = function (s) {
    ...
    if (s.group === CB) {
      ...
      if (v in this.factors) this.factors[v] = _.multiply(this.factors[v], s);
  ...
  Factors.prototype.toSymbol = function () {
    ...
    for (var x in this.factors) {
      var factor = this.factors[x].power.equals(1)
        ? _.symfunction(core.PARENTHESIS, [this.factors[x]])
        : this.factors[x];
  ...
  var __ = (core.Algebra = {
    ...
    proots: function (symbol, decp) {
      ...
      if (symbol instanceof Symbol && symbol.isPoly()) {
        ...
        if (symbol.group === PL) {
          ...
          var lowest_symbol = symbol.symbols[lowest_pow]
            .clone()
            .toUnitMultiplier();
        ...
        if (symbol.group === core.groups.S) {
          ...
          var powers = keys(symbol.symbols),
            ...
            symbol = core.PARSER.divide(
              symbol,
              core.PARSER.parse(symbol.value + "^" + minpower)
            );
        }

        var variable = keys(symbol.symbols).sort().pop(),
          sym =
            symbol.group === core.groups.PL
              ? symbol.symbols
      ...
      function calcroots(rarr, powers, max) {
        ...
        if (max > MAXDEGREE) {
          throw new Error(
            "This utility accepts polynomials of degree up to " +
              MAXDEGREE +
              ". "
          );
        }
        ...
        function QuadIT_ak1(
          ...
          do {
            ...
            if (
              Math.abs(Math.abs(iPar.szr) - Math.abs(iPar.lzr)) >
              0.01 * Math.abs(iPar.lzr)
            )
              break;
            ...
            mp =
              Math.abs(-(iPar.szr * sdPar.b) + sdPar.a) +
            ...
            ee =
              (9.0 * ee +
                2.0 * Math.abs(t) -
                7.0 * (Math.abs(sdPar.a + t) + zm * Math.abs(sdPar.b))) *
              DBL_EPSILON;
        ...
        function RealIT_ak1(DBL_EPSILON, iPar, sdPar, N, p, NN, qp, K, qk) {
          ...
          for (;;) {
            ...
            t =
              Math.abs(kv) > Math.abs(K[nm1]) * 10.0 * DBL_EPSILON
                ? -(pv / kv)
                : 0.0;
            s += t;
          }
        ...
        function rpSolve(degPar, p, zeror, zeroi) {
          ...
          while (N >= 1) {
            ...
            if (
              (sc <= 1.0 && moduli_max >= 10) ||
              (sc > 1.0 && Number.MAX_VALUE / sc >= moduli_max)
            ) {
            ...
            for (jj = 0; jj < 5; jj++) {
              ...
              if (zerok) {
                ...
                K[0] = p[0];
                zerok =
                  Math.abs(K[NM1]) <= Math.abs(bb) * DBL_EPSILON * 10.0 ? 1 : 0;
              }
            }
    ...
    coeffs: function (symbol, wrt, coeffs) {
      ...
      if (symbol.group === EX && symbol.contains(wrt, true))
        _.error(
          "Unable to get coefficients using expression " + symbol.toString()
        );
      ...
      if (vars.length <= 1 && vars[0] === wrt) {
        ...
        if (!wrt)
          _.error(
            "Polynomial contains more than one variable. Please specify which variable is to be used!"
          );
    ...
    Factor: {
      ...
      common: function (symbol, factors) {
        try {
          if (symbol.group === CP) {
            ...
            var symbols = _.expand(symbol.clone(), true).collectSymbols(
              null,
              null,
              function (a, b) {
            ...
            for (var x in map) {
              ...
              if (map[x][1].length === symbols.length) {
                ...
                factor = _.multiply(
                  factor,
                  _.pow(new Symbol(x), new Symbol(map[x][0]))
                );
              }
            }
      ...
      factor: function (symbol, factors) {
        if (symbol.group === FN) symbol = core.Utils.evaluate(symbol);
        ...
        try {
          ...
          if (isInt(p)) {
            ...
            if (multiVar) original = symbol.clone();
            ...
            if (multiVar) {
              ...
              if (all_S && all_unit) return _.pow(symbol, _.parse(p));
            }
        ...
        if (symbol.group !== FN) {
          ...
          for (var i = 0; i < vars.length; i++) {
            do {
              ...
              var d = __.Factor.coeffFactor(
                core.Calculus.diff(symbol, vars[i])
              );
        ...
        if (symbol.group === FN) {
          if (symbol.fname === "sqrt") {
            ...
            factors2.each(function (x) {
              symbol = _.multiply(
                symbol,
                _.parse(core.Utils.format("sqrt({0})", x))
              );
            });
    ...
    gcd: function (a, b) {
      if (
        (a.group === S && b.group === S && a.value !== b.value) ||
        a.group === EX ||
        b.group === EX
      )
    ...
    div: function (symbol1, symbol2) {
      ...
      var vars = core.Utils.arrayUnique(
          variables(symbol1).concat(variables(symbol2))
        ),
        quot,
        rem;
      if (vars.length === 1) {
        ...
        var target =
          is_larger(s1[0], s2[0]) && s1[0].count > s2[0].count ? s2 : s1; //since the num is already larger than we can get the det from denom
